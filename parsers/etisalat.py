...
def normalize_etisalat(df):
    df = df[df['SERVICE'].str.lower() == 'voice']
    df_cleaned = pd.DataFrame()
    df_cleaned["A_NUMBER"] = df.get("ORIGIN NUMBER")
    df_cleaned["B_NUMBER"] = df.get("DESTINATION NUMBER")
    df_cleaned["FULL_DATE"] = pd.to_datetime(df.get("START DATE"), errors='coerce')
    df_cleaned["ROUNDED_VOLUME"] = pd.to_numeric(df.get("DURATION"), errors="coerce")
    df_cleaned["B_NUMBER_FIRST_NAME"] = None
    df_cleaned["B_NUMBER_LAST_NAME"] = None
    df_cleaned["B_NUMBER_ADDRESS"] = df.get("DESTINATION ADDRESS")
    df_cleaned["A_NUMBER_ADDRESS"] = df.get("ORIGIN ADDRESS")
    return df_cleaned

def analyze_location_etisalat(df):
    if df.empty or "A_NUMBER" not in df.columns or "FULL_DATE" not in df.columns or "Site Address" not in df.columns:
    ...
    overall = df_user["Site Address"].value_counts().idxmax()
    overall_count = df_user["Site Address"].value_counts().max()

    work_df = df_user[df_user["HOUR"].between(8, 17)]
    work_loc = work_df["Site Address"].value_counts().idxmax() if not work_df.empty else "لا يوجد"
    work_count = work_df["Site Address"].value_counts().max() if not work_df.empty else 0

    sleep_df = df_user[df_user["HOUR"].between(0, 8)]
    sleep_loc = sleep_df["Site Address"].value_counts().idxmax() if not sleep_df.empty else "لا يوجد"
    sleep_count = sleep_df["Site Address"].value_counts().max() if not sleep_df.empty else 0
...
def last_calls_etisalat(df):
    ...
    for _, row in df_filtered.head(10).iterrows():
        ...
        nid = safe_extract(pd.DataFrame([row]), "ID Number") if direction == "صادرة" else "غير متوفر"
        address = safe_extract(pd.DataFrame([row]), "B_Number_Address") if direction == "صادرة" else "غير متوفر"

        site_user = row.get("Site Address") or "غير متوفر"
        site_other = row.get("B_Number_MU_Site_Address") if direction == "صادرة" else row.get("Site Address")
